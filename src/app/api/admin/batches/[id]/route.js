...
// ✅ PATCH /api/admin/batches/:id  { name?, is_active? }
export async function PATCH(req, { params }) {
  try {
    ...
    console.error("ADMIN BATCHES PATCH ERROR:", err);
...
export async function DELETE(req, { params }) {
  try {
    ...
    await query("DELETE FROM batches WHERE id = ?", [params.id]);
    return NextResponse.json({ message: "Deleted" }, { status: 200 });
  } catch (err) {
    console.error("ADMIN BATCHES DELETE ERROR:", err);
    return NextResponse.json({ message: err?.message || "Error" }, { status: 500 });
  }
}
